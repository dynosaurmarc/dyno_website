...
export default function DynoRechner(props: {
  ...
  const setNum = (k: keyof typeof form, v: string) => setForm((prev) => ({ ...prev, [k]: Number(v) }))

  const av = comparison.avDepot
  const bav = comparison.bav

  return (
    <div style={{ fontFamily: "Inter, system-ui, sans-serif", background: tokens.bg, color: tokens.text, borderRadius: tokens.radius, boxShadow: DesignTokens.shadow, padding: 20, border: `1px solid ${DesignTokens.border}` }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 16, background: DesignTokens.softBg, padding: 6, borderRadius: 999 }}>
        <button onClick={() => setTab("depot")} style={segStyle(tab === "depot", tokens.primary)}>Altersvorsorgedepot (gefördert)</button>
        <button onClick={() => setTab("compare")} style={segStyle(tab === "compare", tokens.primary)}>Vergleich mit DYNO bAV</button>
      </div>

      <div style={grid2}>
        <Field label="Nettoaufwand pro Monat" help="Ihr tatsächlicher Nettoaufwand.">
          <input type="number" value={form.monthlyNetOutlay} min={1} onChange={(e) => setNum("monthlyNetOutlay", e.target.value)} style={inputStyle} />
        </Field>
      ...
      <details open={expanded} onToggle={(e) => setExpanded((e.target as HTMLDetailsElement).open)} style={{ margin: "14px 0" }}>
        ...
        <div style={{ marginTop: 12, ...grid2 }}>
          <Field label="Erwartete Rendite p.a."><input type="number" step="0.001" value={form.expectedReturnPa} onChange={(e) => setNum("expectedReturnPa", e.target.value)} style={inputStyle} /></Field>
          <Field label="Produktkosten p.a. (Effektivkosten)"><input type="number" step="0.001" value={form.totalFeePa} onChange={(e) => setNum("totalFeePa", e.target.value)} style={inputStyle} /></Field>
          <Field label="Grenzsteuersatz" help="Einzahlungsphase"><input type="number" step="0.01" value={form.marginalTaxRate} onChange={(e) => setNum("marginalTaxRate", e.target.value)} style={inputStyle} /></Field>
          <Field label="Steuersatz in Rentenphase"><input type="number" step="0.01" value={form.taxRateInRetirement} onChange={(e) => setNum("taxRateInRetirement", e.target.value)} style={inputStyle} /></Field>
          <Field label="Zulagen Stufe 1 (Rate/CAP)"><div style={{ display: "flex", gap: 8 }}><input type="number" step="0.01" value={form.allowanceTier1Rate} onChange={(e) => setNum("allowanceTier1Rate", e.target.value)} style={inputStyle} /><input type="number" value={form.allowanceTier1Cap} onChange={(e) => setNum("allowanceTier1Cap", e.target.value)} style={inputStyle} /></div></Field>
          <Field label="Zulagen Stufe 2 (Rate/CAP)"><div style={{ display: "flex", gap: 8 }}><input type="number" step="0.01" value={form.allowanceTier2Rate} onChange={(e) => setNum("allowanceTier2Rate", e.target.value)} style={inputStyle} /><input type="number" value={form.allowanceTier2Cap} onChange={(e) => setNum("allowanceTier2Cap", e.target.value)} style={inputStyle} /></div></Field>
          <Field label="Besteuerung Rentenphase"><select value={taxationMode} onChange={(e) => setTaxationMode(e.target.value as TaxationMode)} style={inputStyle}><option value="tax_on_gains">nur Erträge besteuern</option><option value="fully_deferred">voll nachgelagert</option></select></Field>
          {taxationMode === "fully_deferred" && <Field label="Persönlicher Steuersatz (voll nachgelagert)"><input type="number" step="0.01" value={form.fullyDeferredTaxRate} onChange={(e) => setNum("fullyDeferredTaxRate", e.target.value)} style={inputStyle} /></Field>}

          <Field label="bAV-Modus"><select value={bavMode} onChange={(e) => setBavMode(e.target.value as BavInputMode)} style={inputStyle}><option value="simple">Simple (Netto -> Brutto Faktor)</option><option value="detailed">Detailed</option></select></Field>
          <Field label="Arbeitgeberzuschuss"><input type="number" step="0.01" value={form.employerSubsidyRate} onChange={(e) => setNum("employerSubsidyRate", e.target.value)} style={inputStyle} /></Field>
          {bavMode === "simple" ? <Field label="Netto->Brutto Faktor"><input type="number" step="0.0001" value={form.netToGrossFactor} onChange={(e) => setNum("netToGrossFactor", e.target.value)} style={inputStyle} /></Field> : <Field label="Bruttoumwandlung €/Monat"><input type="number" value={form.grossConversion} onChange={(e) => setNum("grossConversion", e.target.value)} style={inputStyle} /></Field>}
          <Field label="Einkommensteuer in Rente (bAV)"><input type="number" step="0.01" value={form.incomeTaxRateRetirement} onChange={(e) => setNum("incomeTaxRateRetirement", e.target.value)} style={inputStyle} /></Field>
          <Field label="KV/PV in Rente (bAV)"><input type="number" step="0.01" value={form.kvPvRateRetirement} onChange={(e) => setNum("kvPvRateRetirement", e.target.value)} style={inputStyle} /></Field>
        </div>
      </details>
      ...
      <div style={cards}>
        <ResultCard title="Altersvorsorgedepot" lines={[
        ...
        {(tab === "compare") && <ResultCard title="DYNO bAV" lines={[
      ...
      {tab === "compare" && <div style={{ marginTop: 14, fontWeight: 700, color: comparison.winner === "bav" ? DesignTokens.success : DesignTokens.danger }}>
        Delta: {comparison.winner === "equal" ? "Gleichstand" : comparison.winner === "bav" ? `bAV +${fmt(comparison.deltaNet)}` : `Depot +${fmt(Math.abs(comparison.deltaNet))}`}
      </div>}
...
function Field({ label, children, help }: { label: string; children: React.ReactNode; help?: string }) {
  return <label style={{ display: "grid", gap: 6, fontSize: 14 }}>
    <span style={{ fontWeight: 600 }}>{label}{help ? ` ⓘ ${help}` : ""}</span>{children}
  </label>
}

function ResultCard({ title, lines }: { title: string; lines: [string, string][] }) {
  return <div style={{ border: "1px solid #EAECF0", borderRadius: 14, padding: 14, background: "#fff" }}>
    <div style={{ fontWeight: 700, marginBottom: 8 }}>{title}</div>
    {lines.map(([k, v]) => <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "4px 0", gap: 12 }}><span style={{ color: "#475467" }}>{k}</span><strong>{v}</strong></div>)}
  </div>
}
